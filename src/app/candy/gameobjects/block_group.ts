...
export default class BlockGroup extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Game, x: number, y: number, w = 2, h = 3, color = 'blue', defaultVelocity = 100) {
    super(scene, x, y)
  ...
  */
  update() {
    ...
    if (!this.W || !this.A || !this.S || !this.D || !this.cursor) return
    if (
      (Phaser.Input.Keyboard.JustUp(this.S) || Phaser.Input.Keyboard.JustUp(this.cursor.down)) &&
      this.canMoveDown()
    ) {
      ...
      (Phaser.Input.Keyboard.JustUp(this.W) || Phaser.Input.Keyboard.JustUp(this.cursor.up)) &&
      this.canMoveUp()
    ) {
      ...
      (Phaser.Input.Keyboard.JustUp(this.D) || Phaser.Input.Keyboard.JustUp(this.cursor.right)) &&
      this.canMoveRight()
    ) {
      ...
      (Phaser.Input.Keyboard.JustUp(this.A) || Phaser.Input.Keyboard.JustUp(this.cursor.left)) &&
      this.canMoveLeft()
    ) {
